...
| Seed/period multiplicity read as significance | Only two frozen seeds and k=7 periods (Amendment A1; the original "≤3 periods" wording predates A1 and is superseded), all reported; uncertainty context restricted to the predeclared block bootstrap; no new significance machinery; criteria are sign/count rules, not p-values. |
