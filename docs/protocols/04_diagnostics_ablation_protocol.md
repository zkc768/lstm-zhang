...
04_failure_slices.csv:
  ...
  support_down, scope
04_sentinel_summary.csv:
  candidate_role, candidate_id, seed, n_rows, n_blocks, observed_delta,
  label_shuffle_mean, label_shuffle_p95, label_shuffle_p_value,
  time_reverse_delta, n_perm, sentinel_status, scope
  (leakage negative control; sentinel_status="not_computed" when baseline
  reconstruction did not verify — a sentinel against the wrong baseline would
  be meaningless; measure-only, never re-scores)
04_ablation_plan_ledger.csv:
...
- Durable save: nb04 validates the thirteen `REQUIRED_STAGE04_ARTIFACTS`
