...
6. External ARS / academic-research-skills workflows are advisory orchestration
   only. Their human checkpoints, integrity gates, read-only review, and
   claim/citation checks may be borrowed, but they must not override
   `AGENTS.md`, the claims ledger, artifact checksums, run manifests,
   no-holdout/test rules, or the paper contracts.
...
| English drafting / CN-to-EN translation | mandatory pre-read: `docs/lst_models_paper_translation_and_anti_ai_style_guide.md` + `paper/outline_and_claims.md` | Any task that writes or translates paper prose in English. The guide overrides skill-internal style defaults and requires claim_id/evidence_domain/estimand binding before empirical prose. |
| Paper prose drafting (content/narrative) | mandatory pre-read: `docs/lst_models_paper_narrative_and_template_papers_guide.md` (Doc B) + `paper/outline_and_claims.md` | Story line, contribution claims, section page budgets, template-paper usage, Tier-V/control/Tier-G guarded wording, and claims-ledger rules. Read together with the translation guide. |
| Paper figures / tables / LaTeX / submission packaging | mandatory pre-read: `docs/lst_models_paper_format_and_figures_contract.md` (Doc A) + `paper/references.bib` when citations are touched | ICAIF 8-page sigconf constraints, float quotas, exact artifact root/run id, matplotlib style, booktabs rules, double-blind grep gates, GenAI usage statement, citation provenance. |
