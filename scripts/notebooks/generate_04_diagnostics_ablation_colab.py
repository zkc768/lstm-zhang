...
CELL_SPECS = [
    (
        ...
        "Route: `lst_models` V2\n"
        "Scope: `validation_only`; official validation contact is "
        "`read_frozen_artifacts_only` (the frozen Stage 03 prediction dump is "
        "read and summarized; zero new validation fit-predict events)\n"
        "User-facing notebook: `notebooks/04_diagnostics_ablation_colab.ipynb`\n\n"
        "This notebook executes Stage 04 as pre-registered in "
        "`docs/protocols/04_diagnostics_ablation_protocol.md` (D3 core frozen "
        "2026-06-09, operational sections completed 2026-06-10). Stage 04 is "
        "diagnosis, not selection: it cannot change the Stage 03 outcome, and "
        "its findings become limitation text or pre-registered V2.1 items.\n",
    ),
    (
        "markdown",
        ...
        "slices with leave-one-slice-out concentration flags, and failure "
        "tables — all computed from the frozen `03_validation_predictions.csv` "
        "dump. No calibrator fitting, no operating point.\n"
        "- Same-row slice deltas use the deterministic stratified-dummy "
        "reconstruction with dual equality gates against the frozen Stage 03 "
        "artifacts; on any mismatch the affected deltas are emitted as "
        "`not_computed_due_to_baseline_reconstruction_mismatch` (realized and "
        "expectation semantics never mix).\n"
        "- Ablation arm (train-inner only): the four predeclared architectural "
        "controls (`tcn_only`, `dlinear_only`, `last_step_mlp`, "
        "`last_step_lightgbm_control`) fit on the Stage 02 fold/row contract "
        "with hash-verified same-row identity; budget "
        "4 controls x 1 candidate x 3 folds x 2 seeds = 24 <= 32. The frozen "
        "primary is never refit; its train-inner evidence is joined read-only "
        "from the frozen Stage 02 trial ledger.\n"
        "- Controls may not become thesis candidates; the summary carries no "
    ...
    (
        ...
        "Per-control recovery checkpoints are mirrored to "
    ...
    (
        ...
        "PROJECT_REPO_COMMIT = \"8f62d87b683dc1a71805b2b884edd8d7b6fb0dc7\"\n"
    ...
    (
        ...
        "This cell loads the Stage 04 config sidecar, injects the notebook "
        ...
        "frozen blocks. It does not read the dump or fit anything.\n",
    ),
    (
        "code",
        "try:\n"
    ...
    (
        ...
        "Stage 04 verifies the full frozen chain before anything runs. If the "
        "Colab runtime was restarted, this cell downloads the four exact "
        "upstream run folders by Drive path parts (duplicate Drive matches "
        "are a hard error) and the raw ticker files by Drive file ID. The "
        "Stage 03 folder includes the frozen prediction dump (~44 MB).\n",
    ),
    (
        "code",
    ...
    (
        ...
        "Protocol section 13: estimate the materialized float32 bytes for the "
        "capped ablation fold tensors and check the dump is present and "
        "plausibly sized BEFORE any control fit. Abort cleanly with zero fits "
        "if the estimate exceeds the predeclared cap.\n",
    ),
    (
        "code",
        "if RUN_STAGE04:\n"
    ...
    (
        ...
        "The runner writes per-control recovery checkpoints (partial trial "
        "ledger plus `checkpoint_manifest.json`) under the local checkpoint "
        "root. This cell defines a background mirror that uploads "
        "mtime-stable checkpoint files to "
        "`My Drive/lst_models/checkpoints/04_diagnostics_ablation/<run_id>/` "
        "during the run. Checkpoints are recovery state only, never evidence "
        "artifacts.\n",
    ),
    (
        "code",
    ...
    (
        ...
        "`lst_models.stages.diagnostics_ablation.run_stage`. The diagnostics "
        "arm only reads the frozen dump; the ablation arm fits the four "
        "predeclared controls on hash-verified Stage 02 train-inner rows. No "
        "model is ever fit, predicted, thresholded, or calibrated on official "
        "validation rows. The committed notebook remains inert by default.\n",
    ),
    (
        "code",
        "if RUN_STAGE04:\n"
    ...
    (
        ...
        "Immediately after `run_stage` returns, this cell validates the twelve "
        "required Stage 04 outputs (imported from the stage module, never "
        "retyped), refuses upload unless the run manifest records "
        "`new_validation_fit_predict_events=0`, "
        "`official_validation_contact=read_frozen_artifacts_only`, and "
        "`holdout_test_contact=false`, and uploads the run folder to "
        "`My Drive/lst_models/results/04_diagnostics_ablation/<run_id>/`. "
    ...
    (
        "markdown",
        "## Diagnostics Display\n\n"
        "After an approved run, display only Stage 04 artifacts from "
        "`result.run_dir`: the diagnostics report summary, the ablation "
        "summary against the read-only Stage 02 reference rows, compact "
        "calibration and selective tables, and whole-curve plots. Nothing "
        "here marks, recommends, or selects an operating point.\n",
    ),
    (
        "code",
    ...
    (
        ...
        "Stage 04 is diagnosis, not selection. Allowed wording: "
        "`validation-only evidence`, `official validation readout`, "
        "`candidate met/did not meet predeclared validation-readout "
        "criteria`.\n\n"
        ...
        "or a chosen threshold from this notebook. Ablation controls may not "
        "be promoted to thesis candidates; `no_final_model_selected=true` is "
        "retained in every Stage 04 output.\n\n"
        "Required context sentences for any summary based on this run:\n\n"
        "- Stage 04 read and summarized the frozen Stage 03 validation "
        "prediction dump (read contact is recorded in the manifest as "
        "`official_validation_rows_read`); it produced zero new validation "
        "fit-predict events.\n"
        "- Ablation absolute metrics come from capped train-inner folds and "
        "are not comparable to Stage 03 absolute metrics (Zadrozny 2004); "
        "only same-row deltas within each evidence tier support claims.\n"
        "- `activity_tercile` is a dump-native band-pass activity proxy, not "
        "realized volatility; a raw-bar volatility slice is a deferred V2.1 "
        "item, as is SHAP/permutation importance.\n"
        "- Boundary calendar years are partial periods; read slice rows with "
        "their `n_rows` column.\n"
        "- If the baseline reconstruction status is "
        "`mismatch_deltas_not_computed`, the affected slice deltas and "
        "leave-one-out flags are intentionally absent rather than "
        "approximated; only the frozen Stage 03 per-ticker deltas remain "
        "authoritative.\n",
    ),
]
...
def main() -> None:
    ...
    output_path = root / "notebooks" / "04_diagnostics_ablation_colab.ipynb"
    output_path.parent.mkdir(parents=True, exist_ok=True)
