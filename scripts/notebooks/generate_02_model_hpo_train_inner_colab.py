...
CELL_SPECS = [
    ...
    (
        ...
        "run folder. Stage 02 is train-inner HPO only: it does not read, rank, "
        "tune against, or summarize official validation, test, or holdout rows.\n\n"
        "The current executable layer is an HPO planning scaffold. It builds "
        "planned rows from Stage 01 candidate inputs, approved families, search "
        "profiles, train-inner folds, and seeds. It marks those rows as "
        "`skipped_not_implemented`, writes no frozen params, and keeps "
        "`ready_for_stage03=false`.\n\n"
        "Stage 02 must run all Stage 01-approved families that are enabled in "
        "the config, or block if the budget does not close. It must not secretly "
        "drop or demote approved families. Current enabled HPO families:\n\n"
        "- `lightgbm`\n"
        "- `standard_dlinear`\n"
        "- `tcn`\n"
        "- `ms_dlinear_tcn`\n\n"
        "`simple_gru` and `shallow_lstm` remain optional fixed controls, not "
        "core HPO families. Stage 00 baseline registry names remain "
        "`stratified_dummy_train_prior`, `majority_train_prior`, `constant_up`, "
        "and `constant_down`.\n",
    ),
    (
        "markdown",
        ...
        "Formal HPO outputs such as `02_hpo_trial_ledger.csv`, "
        "`02_hpo_summary.csv`, `02_baseline_control_summary.csv`, and "
        "`02_frozen_candidate.json` are future-layer artifacts. They are not "
        "active until config, code, notebook, and tests are updated together.\n",
    ),
    (
        "code",
        "from pathlib import Path\n"
    ...
    (
        ...
        "When `RUN_STAGE02=True` or `RUN_STAGE02_CHECKPOINT=True`, this cell first checks the runtime path. If files "
    ...
    (
        ...
        "Colab runtimes are temporary, so this notebook can write a compact "
        "checkpoint archive to Google Drive when `RUN_STAGE02_CHECKPOINT=True`. "
        "The checkpoint uses the Drive API and a compressed archive instead of "
        "running HPO directly from a mounted Drive folder. This keeps Drive I/O "
        "small and preserves enough Stage 02-safe state to reconstruct the run "
        "after a Colab reset.\n\n"
        "Current scaffold checkpoints contain Stage 02 sidecars, search spaces, "
        "available Stage 01 input artifacts, and optionally the Stage 02 run "
        "folder after `run_stage` returns. Formal long-running HPO still needs "
        "runner-level incremental trial checkpoints.\n",
    ),
    (
        "code",
    ...
    (
        ...
        "the canonical durable results path in Google Drive:\n\n"
        "```text\n"
        "My Drive/lst_models/results/02_model_hpo_train_inner/<stage02_run_id>/\n"
        "```\n\n"
        "This is separate from optional checkpoint archives. Downstream stages "
        "should read result artifacts from this results path, not from "
        "`/content/...` runtime paths and not from checkpoint zips.\n",
    ),
    (
        "code",
    ...
    (
        ...
        "Allowed wording only after a future formal HPO implementation writes "
        "formal frozen-candidate artifacts:\n\n"
