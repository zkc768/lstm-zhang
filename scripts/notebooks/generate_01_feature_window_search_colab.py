...
CELL_SPECS = [('markdown',
 ...
 ('code',
  ...
  'PROJECT_BOOTSTRAP_MODE = "github_commit"  # github_commit | drive_bundle | manual_upload | '
  'already_present\n'
  '\n'
  ...
  'STAGE01_DRIVE_RESULT_PATH_PARTS = ["lst_models", "results", "01_feature_window_search"]\n'
  '\n'
  ...
  'print("STAGE01_DRIVE_RESULT_PATH_PARTS:", STAGE01_DRIVE_RESULT_PATH_PARTS)\n'
  'print("RUN_PROJECT_BOOTSTRAP:", RUN_PROJECT_BOOTSTRAP)\n'
  'print("RUN_RAW_DOWNLOAD:", RUN_RAW_DOWNLOAD)\n'
  'print("RUN_STAGE00_DRIVE_SYNC:", RUN_STAGE00_DRIVE_SYNC)\n'
  'print("RUN_STAGE01:", RUN_STAGE01)\n'
  'print("RUN_STAGE01_DRIVE_BACKUP:", RUN_STAGE01_DRIVE_BACKUP)\n'
 ...
 ('code',
  ...
  'stage_outputs = stage01_config["outputs"]\n'
  'stage00_inputs["stage00_run_id"] = STAGE00_RUN_ID\n'
  'stage00_inputs["stage00_runtime_run_dir"] = str(STAGE00_OUTPUT_DIR)\n'
  'stage00_inputs["stage00_drive_path_parts"] = STAGE00_DRIVE_PATH_PARTS\n'
  'stage00_inputs["stage00_run_manifest"] = str(STAGE00_OUTPUT_DIR / "run_manifest.json")\n'
  'stage00_inputs["raw_data_dir"] = str(RAW_DATA_DIR)\n'
  ...
  'assert stage00_inputs["raw_data_dir"] == str(RAW_DATA_DIR)\n'
  'assert stage_outputs["output_dir"] == str(OUTPUT_DIR)\n'
  ...
  'assert stage01_config["optional_fixed_controls"]["shallow_lstm"]["stage02_hpo_family"] is '
  'False\n'
  '\n'
  'print(json.dumps({\n'
  ...
  '    "raw_data_dir": stage00_inputs["raw_data_dir"],\n'
 ...
 ('markdown',
  '## Raw Data Check\n'
  '\n'
  'Stage 01 builds feature/window tensors from the five authoritative raw\n'
  '1-minute `.txt` files. The notebook downloads by fixed Google Drive file\n'
  'ID only when `RUN_RAW_DOWNLOAD=True`; it does not scan Drive folders.\n'),
 ('code',
  'with RAW_DATA_MANIFEST_PATH.open("r", encoding="utf-8") as handle:\n'
  '    raw_data_manifest = yaml.safe_load(handle)\n'
  '\n'
  'raw_files = raw_data_manifest["raw_source"]["files"]\n'
  'expected_raw_paths = {\n'
  '    ticker: RAW_DATA_DIR / spec["name"]\n'
  '    for ticker, spec in raw_files.items()\n'
  '}\n'
  '\n'
  '\n'
  'def download_raw_file_by_id(file_id, output_path):\n'
  '    try:\n'
  '        from google.colab import auth\n'
  '        from googleapiclient.discovery import build\n'
  '        from googleapiclient.http import MediaIoBaseDownload\n'
  '    except ImportError as exc:\n'
  '        raise RuntimeError(\n'
  '            "RUN_RAW_DOWNLOAD=True requires Colab Google API dependencies. "\n'
  '            "Otherwise place raw .txt files at RAW_DATA_DIR before running Stage 01."\n'
  '        ) from exc\n'
  '\n'
  '    auth.authenticate_user()\n'
  '    service = build("drive", "v3")\n'
  ...
  '\n'
  '\n'
  'missing_raw_paths = [path for path in expected_raw_paths.values() if not path.exists()]\n'
  'if RUN_STAGE01 and missing_raw_paths:\n'
  '    if RUN_RAW_DOWNLOAD:\n'
  '        for ticker, spec in raw_files.items():\n'
  '            output_path = expected_raw_paths[ticker]\n'
  '            if not output_path.exists():\n'
  '                downloaded = download_raw_file_by_id(spec["file_id"], output_path)\n'
  '                print(f"Downloaded raw file for {ticker}: {downloaded}")\n'
  '    else:\n'
  '        missing_text = "\\n".join(f"- {path}" for path in missing_raw_paths)\n'
  '        raise FileNotFoundError(\n'
  '            "Stage 01 raw files are missing. Set RUN_RAW_DOWNLOAD=True in the "\n'
  '            "first cell, or place the files at RAW_DATA_DIR. Missing files:\\n"\n'
  '            + missing_text\n'
  '        )\n'
  '\n'
  'if RUN_STAGE01:\n'
  '    missing_after = [path for path in expected_raw_paths.values() if not path.exists()]\n'
  '    if missing_after:\n'
  '        raise FileNotFoundError("\\n".join(str(path) for path in missing_after))\n'
  '    print("Raw data presence check passed.")\n'
  '    print(json.dumps({ticker: str(path) for ticker, path in expected_raw_paths.items()}, indent=2))\n'
  'else:\n'
  '    print("RUN_STAGE01=False; raw data check not executed.")\n'),
 ('markdown',
 ...
 ('code',
  ...
  '\n'
  'if RUN_STAGE01:\n'
  '    missing_before = [name for name in required_stage00_artifacts if not (STAGE00_OUTPUT_DIR / '
  'name).exists()]\n'
 ...
 ('code',
  '# Stage 01 Drive Result Backup\n'
  'def get_drive_service_for_stage01_result_backup():\n'
  '    try:\n'
  '        from google.colab import auth\n'
  '        from googleapiclient.discovery import build\n'
  '    except ImportError as exc:\n'
  '        raise RuntimeError("RUN_STAGE01_DRIVE_BACKUP=True only works inside Colab with Google API dependencies.") from exc\n'
  '    auth.authenticate_user()\n'
  '    return build("drive", "v3")\n'
  '\n'
  '\n'
  'def find_stage01_result_drive_child(service, parent_id, name, mime_type=None):\n'
  '    escaped_name = quote_drive_query_value(name)\n'
  ...
  '\n'
  '\n'
  'def ensure_stage01_result_drive_folder(service, parent_id, name):\n'
  '    folder_mime = "application/vnd.google-apps.folder"\n'
  '    matches = find_stage01_result_drive_child(service, parent_id, name, folder_mime)\n'
  '    if len(matches) == 1:\n'
  '        return matches[0]["id"]\n'
  '    if len(matches) > 1:\n'
  ...
  '\n'
  '\n'
  'def ensure_stage01_result_drive_path(service, path_parts):\n'
  '    folder_id = "root"\n'
  '    for part in path_parts:\n'
  '        folder_id = ensure_stage01_result_drive_folder(service, folder_id, part)\n'
  '    return folder_id\n'
  '\n'
  '\n'
  'def upload_or_update_stage01_result_file(service, drive_folder_id, run_dir, local_path):\n'
  '    from googleapiclient.http import MediaFileUpload\n'
  '\n'
  '    relative_path = local_path.relative_to(run_dir)\n'
  '    matches = find_stage01_result_drive_child(service, drive_folder_id, relative_path.name)\n'
  ...
  '\n'
  '\n'
  'def backup_stage01_results_to_drive(output_run_dir):\n'
  '    from datetime import datetime, timezone\n'
  '    import json\n'
  '\n'
  '    run_dir = Path(output_run_dir)\n'
  '    if not run_dir.exists():\n'
  '        raise FileNotFoundError(f"Stage 01 output folder not found: {run_dir}")\n'
  '    required_stage01_files = [\n'
  '        stage01_config["outputs"]["manifest"],\n'
  '        stage01_config["outputs"]["artifact_inventory"],\n'
  '        stage01_config["outputs"]["summary"],\n'
  '        stage01_config["outputs"]["candidate_inputs"],\n'
  '        stage01_config["outputs"]["probe_ledger"],\n'
  '        stage01_config["outputs"]["fold_manifest"],\n'
  '        stage01_config["outputs"]["label_band_diagnostic"],\n'
  '    ]\n'
  '    missing = [name for name in required_stage01_files if not (run_dir / name).exists()]\n'
  '    if missing:\n'
  '        raise FileNotFoundError(f"Missing required Stage 01 artifacts before Drive backup: {missing}")\n'
  '    run_manifest_path = run_dir / stage01_config["outputs"]["manifest"]\n'
  '    run_manifest = json.loads(run_manifest_path.read_text(encoding="utf-8"))\n'
  '    if run_manifest.get("holdout_test_contact") is not False:\n'
  '        raise ValueError(f"Stage 01 result backup requires holdout_test_contact=false: {run_manifest_path}")\n'
  '    candidate_path = run_dir / stage01_config["outputs"]["candidate_inputs"]\n'
  '    candidate_inputs = json.loads(candidate_path.read_text(encoding="utf-8"))\n'
  '    if candidate_inputs.get("holdout_test_contact") is not False:\n'
  '        raise ValueError(f"Stage 01 result backup requires handoff holdout_test_contact=false: {candidate_path}")\n'
  '    service = get_drive_service_for_stage01_result_backup()\n'
  '    drive_path_parts = STAGE01_DRIVE_RESULT_PATH_PARTS + [run_dir.name]\n'
  '    drive_folder_id = ensure_stage01_result_drive_path(service, drive_path_parts)\n'
  ...
  '    uploads = [upload_or_update_stage01_result_file(service, drive_folder_id, run_dir, path) for path in local_files]\n'
  '    backup_manifest = {\n'
  ...
  '        "no_final_model_selected": candidate_inputs.get("no_final_model_selected"),\n'
  '    }\n'
  '    backup_manifest_path.write_text(json.dumps(backup_manifest, indent=2), encoding="utf-8")\n'
  '    manifest_upload = upload_or_update_stage01_result_file(service, drive_folder_id, run_dir, backup_manifest_path)\n'
  ...
  '    backup_manifest_path.write_text(json.dumps(backup_manifest, indent=2), encoding="utf-8")\n'
  '    upload_or_update_stage01_result_file(service, drive_folder_id, run_dir, backup_manifest_path)\n'
  ...
  '\n'
  '\n'
  'if RUN_STAGE01_DRIVE_BACKUP and RUN_STAGE01:\n'
  '    if "result" not in globals():\n'
  '        raise RuntimeError("RUN_STAGE01_DRIVE_BACKUP=True requires running Stage 01 first.")\n'
  '    stage01_drive_backup_manifest = backup_stage01_results_to_drive(result.output_dir)\n'
  'else:\n'
  '    print("RUN_STAGE01_DRIVE_BACKUP is disabled or RUN_STAGE01=False; no Stage 01 result backup uploaded.")\n'),
 ('markdown',
  '## Artifact Display\n'
  '\n'
 ...
 ('code',
  ...
  '    band_diagnostic_path = run_dir / "01_train_label_band_diagnostic.csv"\n'
  '\n'
  '    for path in [inventory_path, summary_path, candidate_path, ledger_path, band_diagnostic_path]:\n'
  '        require_path(path)\n'
  '\n'
  '    inventory = pd.read_csv(inventory_path)\n'
  '    summary = pd.read_csv(summary_path)\n'
  '    ledger = pd.read_csv(ledger_path)\n'
  '    band_diagnostic = pd.read_csv(band_diagnostic_path)\n'
...
def main() -> None:
    ...
    output_path = root / "notebooks" / "01_feature_window_search_colab.ipynb"
    output_path.parent.mkdir(parents=True, exist_ok=True)
